# 14_Capability_Slice_Map: ответы

---

## Принцип slicing

**Решение:** Slicing по **пользовательскому результату**. Каждый slice — это вертикальный срез, который даёт пользователю ощутимый результат от начала до конца (UI → логика → данные). Не «backend slice + frontend slice», а «пользователь может сделать X и получить Y».
...
**Что должно быть выполнено до начала slicing:**
...
**User outcome:** Пользователь видит общую картину качества модели: сколько orphan элементов, какие слои покрыты плохо, где элементы «невидимы» на диаграммах. Также может работать с моделью через таблицу.
...
**AC:**
- AC-4.1–AC-4.4 (coverage корректен)
- AC-5.3, AC-5.4 (фильтры работают)
...
**User outcome:** Пользователь экспортирует impact подграф в GraphML (для yEd) и таблицу элементов в CSV (для Excel). Может поделиться результатами.
...
**Milestone:** MS-3+ (если хватит времени в неделю 4-5)
...
### Slice S-6 (stretch): «Работаю с расширенными фильтрами»
