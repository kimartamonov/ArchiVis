# 15_Issue_Decomposition_Rules: ответы

---

## Структура backlog

**Решение:** `Milestones → Issues` (без epics). Для MVP из 4 milestones и ~25-35 issues epics — лишний уровень абстракции. Milestones соответствуют MS-1 через MS-4 из Capability Slice Map. Issues привязаны к milestone через GitHub milestones. Labels используются для типизации.
...
НЕ нужные типы для MVP: `refactor` (рефакторим внутри feature issues), `decision` (решения принимаются в документации, не в issues), `validation` (входит в spike).
...
**Запрещено:**
- Безглагольные названия: `Architeezy connector` → непонятно, что делать.
...
- Логически неразделимая работа (connector: fetch + normalize — это один issue).
...
**Как фиксировать:** В описании issue — строка `blocked-by: #N`. В GitHub — можно использовать task lists или mentions.
...
| «Copy-paste issues» | Дубликаты, путаница | Объединить или разделить чётко |
| «Premature optimization»: «Optimize for 10K nodes» | Не нужно для MVP | Пометить label `post-mvp` |
| «Dependency hell»: все issues blocked by всё | Парализует работу | Минимизировать зависимости. Параллельные issues |

---

## Требования к AI-генерации issues

Если issues генерируются AI (Claude, GPT, etc.):

### Входные артефакты:
1. Capability Slice Map (этот документ 14).
2. Functional Requirements (документ 05).
3. Acceptance Criteria (документ 11).
4. Issue Decomposition Rules (этот документ 15).
...
2. Проверить: нет ли «catch-all» issues.
3. Проверить: acceptance criteria конкретны и проверяемы.
4. Проверить: зависимости корректны (нет циклов, нет ложных).
5. Проверить: naming соответствует правилам.
6. Проверить: milestone назначен.

### Чего AI НЕ должен делать:
- Не добавлять issues для post-MVP features.
- Не создавать issues без AC.
- Не придумывать новые requirements (только из документов).
- Не оценивать время (мы не используем estimates).
